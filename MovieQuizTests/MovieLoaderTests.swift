...
struct StubNetworkClient: NetworkRouting {
    ...
    let emulateError: Bool // этот параметр нужен, чтобы заглушка эмулировала либо оишбку сети, либо успешный ответ
    
    func fetch(url: URL, handlerL @escaping (Result<Datam Error>) -> Void) {
        if emulateError {
...
class MovieLoaderTests: XCTestCase {
    func testSuccessLoading() throws {
        ...
        let loader = MovieLoader(stubNetworkClient: stubNetworkClient)
        //When
        ...
        loader.loadMovies{ result in
            ...
            case .success(let movies):
                // предположим что пришло два фильма, ведь в тестовых данных из всего 2
                XCTAssertEqual(movies.items.count, 2)
                expectation.fulfill()
            case .failure(_):
                // мы не ожидаем, что пришла ошибка; если она появится, надо будет провалиь тест
